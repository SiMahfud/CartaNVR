...
function startFFmpegForCamera(camera, startDirWatcherCallback) {
  ...
  proc.on('close', (code, signal) => {
    ...
    if (st && st.manualStop) {
      ...
      st.manualStop = false; // Reset for next time
      return;
    }
...
async function stopFFmpegForCamera(cameraId) {
  ...
  return new Promise(resolve => {
    ...
    // We use SIGTERM to allow for clean exit, but we also clear procState 

    // to prevent auto-restart logic in 'close' event handler.

    const st = procState.get(camId);

    if (st) {

      // Set a flag or just clear it to signal manual stop
