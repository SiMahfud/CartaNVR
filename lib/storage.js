    ...
    try {
        ...
        for (const storage of storages) {
            ...
            if (totalSize > maxStorageBytes) {
                console.log(`[STORAGE] Storage '${storage.name}' is over capacity (${(totalSize / (1024*1024*1024)).toFixed(2)} GB / ${storage.max_gb} GB). Cleaning up...`);
                ...
                while (totalSize > maxStorageBytes && allFiles.length > 0) {
                    ...
                    try {
                        ...
                        console.log(`[STORAGE] Deleted old file: ${oldestFile.filePath}`);
                    } catch (e) {
                        console.warn(`[STORAGE] Failed deleting:`, oldestFile.filePath, e.message);
                    }
                }
            }
        }
