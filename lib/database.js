...
async function getAllCameras() {
  ...
  return rows.map(c => ({...c, is_hevc: !!c.is_hevc}));
}

async function addCamera(camera) {
  const { name, ip_address, rtsp_url, storage_id, is_hevc } = camera;
  const id = await runInsert('INSERT INTO cameras (name, ip_address, rtsp_url, storage_id, is_hevc) VALUES (?, ?, ?, ?, ?)', [name, ip_address, rtsp_url, storage_id, is_hevc ? 1 : 0]);
  return { id, ...camera };
}
...
async function getCameraById(id) {
    ...
    if (row) {
      return {...row, is_hevc: !!row.is_hevc};
    }
    return null;
}
...
async function updateCamera(id, camera) {
  ...
  await runUpdate('UPDATE cameras SET name = ?, rtsp_url = ?, storage_id = ?, is_hevc = ? WHERE id = ?', [name, rtsp_url, storage_id, is_hevc ? 1 : 0, id]);
  return { id, ...camera };
}
