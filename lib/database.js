...
async function init() {
  ...
  if (isMysql) {
    ...
    await db.execute(`CREATE TABLE IF NOT EXISTS cameras (
      ...
      stream_method VARCHAR(20) DEFAULT 'dash',
    ...
    const [methodCol] = await db.execute("SHOW COLUMNS FROM cameras LIKE 'stream_method'");
    if (methodCol.length === 0) {
      await db.execute("ALTER TABLE cameras ADD COLUMN stream_method VARCHAR(20) DEFAULT 'dash'");
    }

    const [audioCol] = await db.execute("SHOW COLUMNS FROM cameras LIKE 'has_audio'");
    if (audioCol.length === 0) {
      await db.execute("ALTER TABLE cameras ADD COLUMN has_audio TINYINT(1) DEFAULT 0");
    }
    ...
    await new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS cameras (
          ...
          stream_method TEXT DEFAULT 'dash',
        ...
        db.all("PRAGMA table_info(cameras)", (err, columns) => {
          if (!err && columns) {
            ...
            if (!columns.some(col => col.name === 'stream_method')) {
              db.run("ALTER TABLE cameras ADD COLUMN stream_method TEXT DEFAULT 'dash'");
            }
            if (!columns.some(col => col.name === 'has_audio')) {
              db.run("ALTER TABLE cameras ADD COLUMN has_audio BOOLEAN DEFAULT 0");
            }
          }
          resolve();
        });
...
async function getAllCameras() {
  ...
  return rows.map(c => ({ ...c, is_hevc: !!c.is_hevc, enabled: !!c.enabled, has_audio: !!c.has_audio }));
}

async function addCamera(camera) {
  const { name, ip_address, rtsp_url, storage_id, is_hevc, enabled, stream_method, has_audio } = camera;
  const id = await runInsert('INSERT INTO cameras (name, ip_address, rtsp_url, storage_id, is_hevc, enabled, stream_method, has_audio) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [name, ip_address, rtsp_url, storage_id, is_hevc ? 1 : 0, enabled !== false ? 1 : 0, stream_method || 'dash', has_audio ? 1 : 0]);
  return { id, ...camera };
}
...
async function getCameraById(id) {
  ...
  if (row) {
    return { ...row, is_hevc: !!row.is_hevc, enabled: !!row.enabled, has_audio: !!row.has_audio };
  }
  return null;
}

async function updateCamera(id, camera) {
  const { name, rtsp_url, storage_id, is_hevc, enabled, stream_method, has_audio } = camera;
  await runUpdate('UPDATE cameras SET name = ?, rtsp_url = ?, storage_id = ?, is_hevc = ?, enabled = ?, stream_method = ?, has_audio = ? WHERE id = ?', [name, rtsp_url, storage_id, is_hevc ? 1 : 0, enabled ? 1 : 0, stream_method || 'dash', has_audio ? 1 : 0, id]);
  return { id, ...camera };
}
