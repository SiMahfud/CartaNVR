...
async function addCamera(camera) {
  const { name, ip_address, rtsp_url, storage_id, is_hevc, enabled, stream_method } = camera;
  const id = await runInsert('INSERT INTO cameras (name, ip_address, rtsp_url, storage_id, is_hevc, enabled, stream_method) VALUES (?, ?, ?, ?, ?, ?, ?)', [name, ip_address, rtsp_url, storage_id, is_hevc ? 1 : 0, enabled !== false ? 1 : 0, stream_method || 'dash']);
  return { id, ...camera };
}
...
async function updateCamera(id, camera) {
  const { name, rtsp_url, storage_id, is_hevc, enabled, stream_method } = camera;
  await runUpdate('UPDATE cameras SET name = ?, rtsp_url = ?, storage_id = ?, is_hevc = ?, enabled = ?, stream_method = ? WHERE id = ?', [name, rtsp_url, storage_id, is_hevc ? 1 : 0, enabled ? 1 : 0, stream_method || 'dash', id]);
  return { id, ...camera };
}
