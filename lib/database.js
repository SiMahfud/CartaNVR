...
db.serialize(() => {
  ...
  db.all("PRAGMA table_info(cameras)", (err, columns) => {
    ...
    const hasIsHevc = columns.some(col => col.name === 'is_hevc');
    if (!hasIsHevc) {
      db.run('ALTER TABLE cameras ADD COLUMN is_hevc BOOLEAN DEFAULT 0', (err) => {
        if(err) console.error("Error adding is_hevc column to cameras:", err);
      });
    }
  });
});
    ...
    return new Promise((resolve, reject) => {
        ...
        db.all(query, [cameraId, startTime, endTime], (err, rows) => {
            ...
            const formattedRows = rows.map(row => {
                ...
                if (camFolderIndex === -1) {
                    return { ...row, file: '' }; // Return empty if path is invalid
                }
...
function deleteAllRecordingsFromDB() {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM recordings', [], function(err) {
            if (err) {
                return reject(err);
            }
            resolve({ deleted: this.changes });
...
function getAllCameras() {
  return new Promise((resolve, reject) => {
    db.all('SELECT c.*, s.name as storage_name, s.path as storage_path FROM cameras c LEFT JOIN storages s ON c.storage_id = s.id ORDER BY c.name ASC', [], (err, rows) => {
      ...
      resolve(rows.map(c => ({...c, is_hevc: !!c.is_hevc})));
    });
  });
}
...
function addCamera(camera) {
  return new Promise((resolve, reject) => {
    const { name, ip_address, rtsp_url, storage_id, is_hevc } = camera;
    db.run('INSERT INTO cameras (name, ip_address, rtsp_url, storage_id, is_hevc) VALUES (?, ?, ?, ?, ?)', [name, ip_address, rtsp_url, storage_id, is_hevc], function(err) {
      if (err) {
        reject(err);
        return;
      }
...
function getCameraById(id) {
    return new Promise((resolve, reject) => {
        db.get('SELECT c.*, s.name as storage_name, s.path as storage_path FROM cameras c LEFT JOIN storages s ON c.storage_id = s.id WHERE c.id = ?', [id], (err, row) => {
            ...
            if (row) {
              resolve({...row, is_hevc: !!row.is_hevc});
            } else {
              resolve(null);
            }
        });
...
function updateCamera(id, camera) {
  return new Promise((resolve, reject) => {
    const { name, rtsp_url, storage_id, is_hevc } = camera;
    db.run('UPDATE cameras SET name = ?, rtsp_url = ?, storage_id = ?, is_hevc = ? WHERE id = ?', [name, rtsp_url, storage_id, is_hevc, id], function(err) {
      if (err) {
        reject(err);
        return;
      }
