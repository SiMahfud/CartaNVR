...
async function init() {
  ...
  if (isMysql) {
    ...
    const [recordingColumns] = await db.execute("SHOW COLUMNS FROM recordings LIKE 'file_size'");
    if (recordingColumns.length === 0) {
      await db.execute("ALTER TABLE recordings ADD COLUMN file_size BIGINT NOT NULL DEFAULT 0");
    }
    ...
    await new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        db.all("PRAGMA table_info(recordings)", (err, columns) => {
          if (!err && columns) {
            if (!columns.some(col => col.name === 'file_size')) {
              db.run('ALTER TABLE recordings ADD COLUMN file_size INTEGER NOT NULL DEFAULT 0');
            }
          }
        });
      });
...
async function addRecording(recording) {
  const { camera_id, file_path, timestamp, duration, file_size } = recording;
  const sql = isMysql
    ? 'INSERT IGNORE INTO recordings (camera_id, file_path, timestamp, duration, file_size) VALUES (?, ?, ?, ?, ?)'
    : 'INSERT OR IGNORE INTO recordings (camera_id, file_path, timestamp, duration, file_size) VALUES (?, ?, ?, ?, ?)';

  const id = await runInsert(sql, [camera_id, file_path, timestamp, duration, file_size || 0]);
  return { id, ...recording };
}
...
async function getRecordingsByCameraId(cameraId) {
  return runQuery('SELECT file_path, file_size FROM recordings WHERE camera_id = ?', [cameraId]);
}
