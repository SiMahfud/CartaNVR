...
function generateConfig() {
    ...
    const yamlContent = [
        ...
        '',
        'rtsp:',
        '  listen: ""',  // Disable RTSP server (we don't need it)
        '',
        'webrtc:',
 ...
 */
async function addStream(camId, rtspUrl) {
    const streamName = `cam_${camId}`;
    try {
        await apiRequest('PUT', `/api/streams?name=${encodeURIComponent(streamName)}&src=${encodeURIComponent(rtspUrl)}`);
